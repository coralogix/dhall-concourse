...
let Swift =
        ...
        , openstack :
            ...
            , allow_reauth : Optional Bool
            , token_id : Optional Text
            }
        }
      : Type

in  { version = { schema = { number : Text } : Type }
    , params =
        { get =
    ...
    , source =
        { schema = < GCS : GCS | Git : Git | S3 : S3 | Swift : Swift > : Type
        , drivers =
            { gcs = { schema = GCS : Type }
            , git = { schema = Git : Type }
            , s3 = { schema = S3 : Type }
            , swift = { schema = Swift : Type }
            }
        }
    }
